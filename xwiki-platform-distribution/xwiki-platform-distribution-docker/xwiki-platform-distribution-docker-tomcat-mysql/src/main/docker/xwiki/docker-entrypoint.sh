...
function configure() {
  ...
  safesed "@replaceextra@" ""${DB_EXTRA_:-}"" /usr/local/tomcat/webapps/ROOT/WEB-INF/hibernate.cfg.xml
  ...
  if [ ! -z ${PLUGINS+x} ]; then
    echo '  Setting custom plugin list...'
    xwiki_set_cfg 'xwiki.plugins' "${PLUGINS}"
  fi
  if [ ! -z ${SUPERADMIN_PWD+x} ]; then
    echo '  Setting superadmin password...'
    xwiki_set_cfg 'xwiki.superadminpassword' "${SUPERADMIN_PWD}"
  fi
  ...
  if [ ! -z ${EXTRA_PROPERTIES+x} ]; then
    echo '  Adding the following additional properties:'
    printf "${EXTRA_PROPERTIES}\n"
